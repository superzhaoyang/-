...
from tkinter import Tk#标准gui接口
...
import os
import ssl
ssl._create_default_https_context = ssl._create_default_https_context #关闭SSL证书验证
...
def resize(w_box, h_box, pil_image):
    ...
    return pil_image.resize((width, height), Image.ANTIALIAS)#返回高质量图片
...
class uiObject:
    ...
    def show_GUI_movie_detail(self):
        ...
        显示 影片详情 界面GUI
        '''
        self.label_img['state'] = NORMAL#全部设置为只读
    ...
    def hidden_GUI_movie_detail(self):
        ...
        显示 影片详情 界面GUI
        '''
        self.label_img['state'] = DISABLED#全部设置为可写
    ...
    def project_statement_show(self, event):
        open("https://www.superzhaoyang.top")
    ...
    def project_statement_lose_focus(self, event):
        self.project_statement.config(fg="#FF0203")
    def B_0_get_focus(self,event):
        self.B_0.config(fg = 'blue',cursor="hand1")
    def B_0_lose_focus(self,event):
        self.B_0.config(fg = "red")
    def B_0_keyword_get_focus(self,event):
        self.B_0_keyword.config(fg = '#9ACD32',cursor = 'hand1')
    def B_0_keyword_lose_focus(self,event):
        self.B_0_keyword.config(fg = '#8B1C62')
    ...
    def ui_process(self):
        ...
        self.root = root
        # 设置窗口位置
        root.title("python项目开发小组：王朝阳，邓颖岚，耿兆环，黄笑菡，孔湛琦")
        self.center_window(root, 1100, 590)
        root.resizable(15, 15)  # 框体大小可调性，分别表示x,y方向的可变性
        # 从排行榜 电影搜索布局开始
        # 容器控件
        labelframe = LabelFrame(root, width=760, height=300, fg = '#8B5742',bg = '#D3D3D3',text="搜索电影")
        labelframe.place(x=5, y=5)
        self.labelframe = labelframe

        # 电影类型
        L_typeId = Label(labelframe, text='电影类型')
        L_typeId.place(x=0, y=6)
        ...
        C_type.place(x=67, y=8)
        self.C_type = C_type


        # 欲获取的电影数量
        L_count = Label(labelframe, text='获取数量=')
        L_count.place(x=145, y=5)
        ...
        T_count.place(x=223, y=7)
        self.T_count = T_count


        # 评分
        L_rating = Label(labelframe, text='影片评分>')
        L_rating.place(x=272, y = 5)
        ...
        B_0 = Button(labelframe, text="从排行榜搜索",fg = '#9A32CD')
        B_0.place(x=565, y=-1)
        self.B_0 = B_0
        ...
        T_vote_keyword.place(x=68, y=37)
        ...
        #lambda表示绑定的函数需要带参数，请勿删除lambda，否则会出现异常
        #thread_it表示新开启一个线程执行这个函数，防止GUI界面假死无响应
        B_0_keyword = Button(labelframe, text="从关键字搜索",fg = '#EEAD0E')
        B_0_keyword.place(x=565, y=33)
        ...
        # 电影详情布局开始
        # 容器控件
        labelframe_movie_detail = LabelFrame(root, fg = '#EEEE00',bg = '#4876FF',text="影片详情")
        labelframe_movie_detail.place(x=675, y=5)
        self.labelframe_movie_detail = labelframe_movie_detail


        # 框架布局，承载多个控件
        frame_left_movie_detail = Frame(labelframe_movie_detail, width=1260,height=560)
        frame_left_movie_detail.grid(row=0, column=0)
        self.frame_left_movie_detail = frame_left_movie_detail

        frame_right_movie_detail = Frame(labelframe_movie_detail, width=1100,height=560)
        frame_right_movie_detail.grid(row=0, column=0)
        frame_right_movie_detail.place(x = 160,y = 0)
        ...
        label_movie_rating_imdb.place(x=0, y=280)
        self.label_movie_rating_imdb = label_movie_rating_imdb

        # 查询按钮
        B_0_imdb = Button(frame_left_movie_detail, text="初始化",fg ='#436EEE')
        B_0_imdb.place(x=10, y=240)
        self.B_0_imdb = B_0_imdb


        #影片名字
        ft = font.Font( weight=font.BOLD)
        label_movie_name = Label(frame_right_movie_detail, text="影片名字", fg='#FF0000', font=ft,anchor=NW)
        label_movie_name.place(x=100, y=0)
        ...
        label_movie_rating.place(x=100, y=30)
        ...
        label_movie_time.place(x=100, y=60)
        ...
        label_movie_type.place(x=100, y=90)
        self.label_movie_type = label_movie_type

        #影片演员
        ft_actor = font.Font(weight = font.BOLD)
        label_movie_actor = Label(frame_right_movie_detail, text="影片演员", wraplength=180,  fg = '#FF82AB',font=ft_actor,anchor=NW)
        label_movie_actor.place(x=100, y=120)
        ...
        labelframe_movie_play_online = LabelFrame(root, width=324, height=230,fg = '#242424', bg = '#CDB79E',text="在线观看")
        ...
        labelframe_movie_save_cloud_disk = LabelFrame(root, width=336, height=230,fg = '#32CD32',bg= '#8B6914', text="云盘搜索")
        labelframe_movie_save_cloud_disk.place(x=338, y=305)
        ...
        # BT下载布局开始
        #change1
        labelframe_movie_bt_download = LabelFrame(root, width=0, height=230, text="影视下载")
        ...
        project_statement = Label(root, text="豆瓣电影小助手(可筛选、下载自定义电影)----python项目于开发小组", fg='#00FFFF', font=ft,anchor=NW)
        ...
        B_0.bind('<Enter>',self.B_0_get_focus)
        B_0.bind('<Leave>',self.B_0_lose_focus)
        B_0_keyword.bind('<Enter>',self.B_0_keyword_get_focus)
        B_0_keyword.bind('<Leave>',self.B_0_keyword_lose_focus)
        root.mainloop()